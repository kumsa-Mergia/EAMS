...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <h1 class="h3 mb-0 text-gray-800"><?= __('Fill Attendance')?></h1>
</div>
...
<div class="card shadow mb-4">
    <div class="card header py-3">
      <h2 class="m-4 font-weight-bold text-primary">
      <div class="well text-center"><?= date("D,M d - Y") ?></div>
...
if(isset($_SESSION['success']) && $_SESSION['success'] !=''){
    echo '<h6 class="text-success font-weight-bold alert">'.$_SESSION['success']. '</h6>';
...
?>
    ...
    <input type="hidden" value="<?php echo $row['employeename']; ?>" name="employeename[]">
    </td>
    <td><?php echo $row['department']; ?>
    <input type="hidden" value="<?php echo $row['department']; ?>" name="department[]">
    </td>
    <td><?php echo $row['shift']; ?>
    <input type="hidden" value="<?php echo $row['shift']; ?>" name="shift[]">
    </td>
    <td>
        
    <input type="radio" value="Present" name="attendance_status[<?php echo $counter; ?>]"> <?= __('Present')?>
    <input type="radio" value="Late" name="attendance_status[<?php echo $counter; ?>]"> <?= __('Late')?>
    <input type="radio" value="Absent" name="attendance_status[<?php echo $counter; ?>]"> <?= __('Absent')?>
